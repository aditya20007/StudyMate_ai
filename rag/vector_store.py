...
from rag.embeddings import EMBEDDING_DIM   # 768 for Groq nomic model


class VectorStore:
    """
    FAISS-based vector store for semantic similarity search.
    Persists index and metadata to disk so data survives restarts.
    """
    ...
    def _load(self):
        try:
            ...
            # Dimension mismatch check — happens if old 384-dim index exists
            if self.index.d != EMBEDDING_DIM:
                logger.warning(
                    f"Index dim {self.index.d} != expected {EMBEDDING_DIM}. Rebuilding."
                )
                self._create_new()
